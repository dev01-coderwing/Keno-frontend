...
function KenoTop10() {
    ...
    const renderDroughtBadge = (value) => {
        ...
        return (
            <span
                className="px-2 py-1 rounded text-sm font-semibold text-white inline-block min-w-[40px] text-center"
                style={{ backgroundColor: bgColor }}
            >
                {value}
            </span>
                ...
                <tbody>
                    {data.map((item, idx) => (
                        <tr key={idx} className="border-t border-gray-700">
                            ...
                            <td className="p-3 text-center">{item.winPercentage}%</td>
                            <td className="p-3 text-center">{item.averageDrought}</td>
