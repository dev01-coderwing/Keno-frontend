...
const BetTypeTable = ({ title, rows }) => {
  ...
  return (
    <div className="bg-[#1D1D1D] rounded-xl mt-6 p-4">
      <h3 className="text-lg font-semibold mb-3">{title} (TOP 10 Combos Last 360 Games)</h3>
...
function TracksideTop10() {
  ...
  return (
    <div className="space-y-6">
      <BetTypeTable
        title="Quinella"
        rows={tracksideTop10?.Quinella?.slice(0, 10)}
      />

      <BetTypeTable
        title="Exacta"
        rows={tracksideTop10?.Exacta?.slice(0, 10)}
      />

      <BetTypeTable
        title="Trifecta"
        rows={tracksideTop10?.Trifecta?.slice(0, 10)}
      />

      <BetTypeTable
        title="First Four"
        rows={tracksideTop10?.["First Four"]?.slice(0, 10)}
      />
    </div>
  );
}
