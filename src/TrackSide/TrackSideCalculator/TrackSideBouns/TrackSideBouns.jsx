...
//   const comm = Number(commission) / 100 || 0; // 0–1

//   if (!B || !back || !lay || back <= 1 || lay <= 1) {
//     setLayStake(0);
//     setProfit(0);
//     setLiability(0);
//     setRetentionPct(0);
...
//   // ✅ Free bet (stake not returned) hedge formula with commission
//   const layStakeCalc = (B * (back - 1)) / (lay - comm);

//   const liabilityCalc = layStakeCalc * (lay - 1);

//   // Profit if BACK wins:
//   const profitBackWins = B * (back - 1) - liabilityCalc;

//   // Profit if LAY wins:
//   const profitLayWins = layStakeCalc * (1 - comm);

//   // Guaranteed profit is the smaller of the two
//   const guaranteedProfit = Math.min(profitBackWins, profitLayWins);

//   const retention = (guaranteedProfit / B) * 100;

//   setLayStake(+layStakeCalc.toFixed(2));
//   setLiability(+liabilityCalc.toFixed(2));
//   setProfit(+guaranteedProfit.toFixed(2));
//   setRetentionPct(+retention.toFixed(2));

// }, [bonusStake, backOdds, layOdds, commission]);
...
// export default TrackSideBouns
    ...
    export default function TrackSideBouns() {
        const [bonusStake, setBonusStake] = useState("");
        const [backOdds, setBackOdds] = useState("");
        const [layOdds, setLayOdds] = useState("");
        const [commission, setCommission] = useState("");
